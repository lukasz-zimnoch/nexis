...
def test_missing_prompt_raises():
    ...
    with patch("sys.argv", ["nexis", "--model", "claude-sonnet-4-6"]):
        with pytest.raises(SystemExit) as exc_info:
            from nexis.__main__ import main
            main()
    assert exc_info.value.code != 0
...
def test_missing_model_raises():
    """Missing --model should cause argparse to exit with error."""
    with patch("sys.argv", ["nexis", "--prompt", "test"]):
        with pytest.raises(SystemExit) as exc_info:
            from nexis.__main__ import main
            main()
    assert exc_info.value.code != 0
...
def test_valid_args_produce_correct_config():
    """Valid CLI args should produce a PipelineConfig with correct values."""
    captured_config = {}
