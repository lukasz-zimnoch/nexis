...
REVIEWER_PROMPTS: dict[ReviewerRole, str] = {
    ...
    ReviewerRole.moat: (
        ...
        "or early demand raise the score. The description will not name its own moat, so "
        "read it out of what the business must do to operate.\n\n"
        "Credit these where the description supports them:\n"
        "- operational data that piles up from ordinary use, where a new entrant has to "
        "run the business as long to hold the same record\n"
        ...
        "Public source material cancels none of these. Judge the assembled result, not "
        "the raw input. A record built from public filings, public rules or public "
        "documents still counts when keeping it current is continuous work and a buyer "
        "cannot get the same thing off the shelf. Do not ask for an input nobody else "
        "can obtain.\n\n"
        "Three traps. Each one removes only the credit it names. A trap that fires never "
        "cancels a different credit you already gave, so apply it and then score what is "
        "left:\n"
